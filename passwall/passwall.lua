...
function index()
    ...
    entry({"admin", "services", appname, "ip"}, call("check_ip")).leaf = true -- PEMBAIKAN #3: Entri menu untuk /ip
    ...
    local com
    if coms then -- PEMBAIKAN #2: Tambah semakan untuk mengelakkan ralat 'nil'
...
function get_ip_geo_info()
    ...
    -- PEMBAIKAN #1: Semak jika JSON parsing berjaya sebelum menggunakan 'info'
    if not info or info.status ~= "success" then
        return {
            flag = "un",
            country = "Unknown",
            ip = "N/A",
...
function get_country_name(countryCode)
    local country_names = {
        ...
        AE = "United Arab Emirates", CO = "Colombia", IN = "India", EG = "Egypt", PE = "Peru"
    }
